...
module.exports = {
        ...
        try {
            ...
            const user_account = await db.users.findOne({
                where: {
                    ...
                    social_type: requestdata.social_type
                }
            })
            ...
            if (user_account) {
                ...
                return helper.success(res, message, user_account)
            } else {
                if (requestdata.email != "") {
                    ...
                    if (user_data) {
                        ...
                        return helper.error501(res, message)
                    }
                }
                var auth_create = crypto.randomBytes(20).toString('hex');
                const create_user = await db.users.create(requestdata);
                let msg = 'Registered successfully';
                data = create_user.toJSON()
                return helper.success(res, "Social Signup succesfully done", create_user)
            }
        } catch (error) {
            console.log(error);
    ...
    otp_verify: async (req, res) => {
        ...
        try {
            ...
            const non_required = {}
            let requestData = await helper.vaildObject(required, non_required, res)
            const otp_verify = await db.users.findOne({
                where: {
                    id: user_id
                }
            })
            let otpSuccessSend = 'Otp verified successfully'

            let otpFailed = 'Please Enter Valid Otp'

            if (otp_verify.otp == requestData.otp) {
                return helper.success(res, "Otp verified successfully.", otpSuccessSend);
            } else {
                return helper.error401(res, "Please Enter Valid Otp .", otpFailed);
            }
        } catch (error) {
            console.log(error);
        }
    ...
    privacy_policy: async (req, res) => {
        try {
            ...
            return helper.success(res, "Privacy Policy ", privacy_policy)
        } catch (error) {
            return helper.error401(res, error)
        }
    },
    ...
    terms_and_condition: async (req, res) => {
        try {
            const termAndCondition = await db.cms.findOne({
                where: {
                    id: 1,
                }
            })
            return helper.success(res, "Terms & Conditions", termAndCondition)
        } catch (error) {
            return helper.error401(res, error)
        }
    },
    about_us: async (req, res) => {
        try {
            const about_us = await db.cms.findOne({
                where: {
                    id: 3,
                }
            })
            return helper.success(res, "About Us", about_us)
        } catch (error) {
            return helper.error401(res, error)
        }
    },
